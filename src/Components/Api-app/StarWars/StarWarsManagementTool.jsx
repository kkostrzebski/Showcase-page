...
const StarWarsManagementTool = () => {
			...
			try {
				...
				if (remainingPages.length > 0) {
					const additionalResponses = await Promise.all(
						remainingPages.map(page => axios.get(`https://swapi.dev/api/people/?page=${page}`))
					)
	...
	return (
		<div className='StarWarsManagementTool'>
			...
			{databaseLoaded ? <p>Database loaded. Total people in database: {totalPeople}</p> : <p>Loading database...</p>}
